...
function Field({
  ...
  return (
    <label className="flex flex-col gap-1">
      <span className="jv-label" style={{ marginBottom: 0 }}>{label}</span>
      <input type="number" step={step} value={value} onChange={(e) => onChange(e.target.value)} className="jv-input" />
    </label>
  );
}
...
export function OptionsCalculatorTab() {
  ...
  return (
    <div className="jarvis">
      ...
      <button onClick={calculate} disabled={loading} className="jv-btn mt-4">
      ...
      {data && (
        <div className="mt-8 grid grid-cols-1 sm:grid-cols-2 gap-8">
          <section>
            <div className="jv-strip-title">Call</div>
          ...
          <section>
            <div className="jv-strip-title">Put</div>
            <div className="grid grid-cols-2 gap-3">
