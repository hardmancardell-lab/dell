...
const BIG_DAY_THRESHOLD_PCT = 5; // disclosed, fixed — not fit to any specific occurrence
...
export interface PostBigDayOccurrence {
  triggerDateKey: string; // the real big-up day itself
...
async function studyOneTicker(ticker: string): Promise<PostBigDayResult> {
  const now = Date.now();
  ...
  for (let i = 1; i < daily.length - 1; i++) {
    ...
    if (triggerDayReturnPct < BIG_DAY_THRESHOLD_PCT) continue;

    const next = daily[i + 1];
    if (trigger.close <= 0) continue;
    occurrences.push({
  ...
  // Intraday checkpoints — real minute bars, but only reach back ~3 months,
  // so only recent occurrences can contribute here regardless of how far
  // back the daily-bar occurrence list itself goes.
  ...
  const dataLimitations: string[] = [
    `"Big day" = a real close-to-close gain of at least ${BIG_DAY_THRESHOLD_PCT}% — a fixed, disclosed threshold, not fit to any single occurrence (including the specific day that prompted this check).`,
  ...
  return {
    ticker,
    bigDayThresholdPct: BIG_DAY_THRESHOLD_PCT,
 ...
 */
export async function runPostBigDayStudy(tickers: string[]): Promise<PostBigDayResult[]> {
  return Promise.all(
